...
if (( skipSystemLog == 0 )); then
	#--SYS TOMCAT INSTALL_LOG. Collect List of the Log Files in the range.
	...
	if [[ -n "$SysLogNames" ]]; then
		...
		echo -e "~INSTALL logs in the range: ${RangeHeadDate} ${RangeHeadTime} .. ${RangeTailDate} ${RangeTailTime}:${LogsParsedInfo}" >> ${FileSeismoErrorsDensity}
	...
	#		date_range_full | sort >> "${FileSeismoErrorsDensity}"
		...
		echo -e "\n~SYS logs ERRORS EXCERPT in the range: ${RangeHeadDate} ${RangeHeadTime} .. ${RangeTailDate} ${RangeTailTime}:${SysLogNames// /\\n}\n" >> ${SeismoSysLogsErrorsExcerpt}
		grep -Eh ${LeadingDateRegexSlash} ${SysLogNames} | grep -E ${FilterCatalinaXxx} >> ${SeismoSysLogsErrorsExcerpt}
	else
		echo "~No access to INSTALL logs at: ${APP_INST}, or logs in the range not found." >> ${FileSeismoErrorsDensity}
	fi
fi #--skipSystemLog

#--HOME_LOG. Collect List of the Log Files in the range.
elapsed=$(( ($(date +%s%N) - start_time) / 1000000000 )); min=$(( elapsed / 60 )); sec=$(( elapsed % 60 ))
...
chain_logfile_names() {
	local logtypename="$1"
	for ((i=0; i<12; i++)); do
		nextLogPathAndName=${logtypename}
		nextLogOnlyName="${logtypename##*/}"
		if [[ i -gt 0 ]]; then
			nextLogPathAndName+=".${i}"
			nextLogOnlyName+=".${i}"
		fi
		if [[ -f "${nextLogPathAndName}" ]]; then
			#--Check if log file contains target dates range. Get first and last timestamp from file
			FIRST_LINE=$(head -n 100 "${nextLogPathAndName}" | grep -m 1 "^[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}" | awk '{print $1,$2}' | cut -d',' -f1)
			LAST_LINE=$(tac "${nextLogPathAndName}" | grep -m 1 "^[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}" | awk '{print $1,$2}' | cut -d',' -f1)
			...
			timeSpanEpoch=$(awk -v last="$lastLineEpoch" -v first="$firstLineEpoch" 'BEGIN {printf "%.2f", (last - first) / 3600}')
			...
			if [[ ($firstLineEpoch -ge $RangeHeadEpoch && $firstLineEpoch -le $RangeTailEpoch) || ($lastLineEpoch -ge $RangeHeadEpoch && $lastLineEpoch -le $RangeTailEpoch) ||
				...
				HomeLogNamesString+="${nextLogPathAndName}"
				lstOfHomeLogFilesWithInfo+="\n\t${nextLogOnlyName}\t [${FIRST_LINE}..${LAST_LINE}]\t ${timeSpanEpoch}h"
				echo -e "InTheRange: ${nextLogOnlyName} * ${FIRST_LINE}..${LAST_LINE}" >/dev/tty #Debug/Verbose
			elif [[ ${is_range_found} -eq 1 ]]; then
				echo -e "OutOfRange: ${nextLogOnlyName} ${FIRST_LINE}..${LAST_LINE}" >/dev/tty #Debug/Verbose
			else
				echo -e "NotInRgYet: ${nextLogOnlyName} ${FIRST_LINE}..${LAST_LINE}" >/dev/tty #Debug/Verbose
			fi
		else
			echo "FileNotFnd: ${nextLogPathAndName}" >/dev/tty #Debug/Verbose
		fi
	done
}
...
if [[ -n $HomeLogNamesString ]]; then
	echo -e "\n~HOME logs from '${APP_HOME}' in range: [${RangeHeadDate} ${RangeHeadTime}..${RangeTailDate} ${RangeTailTime}]:${lstOfHomeLogFilesWithInfo}" >> ${FileSeismoErrorsDensity}
	...
	echo -e "\n~HOME logs ERRORS EXCERPT in the range: ${RangeHeadDate} ${RangeHeadTime} .. ${RangeTailDate} ${RangeTailTime}:\n${lstOfHomeLogFilesWithInfo}\n" >> ${SeismoHomeLogsErrorsExcerpt}
	...
	echo "~No access to HOME logs at: ${APP_HOME}, or logs in the range not found." >> ${FileSeismoErrorsDensity}
fi
...
echo -e "~HOME_LOGS_ALL_IN_TIME_RANGE_CLIP in the range: ${XcrptHeadDateTime} .. ${XcrptTailDateTime}:\n${XcrptFromTheLog// /\\n}" > ${SeismoHomeLogsAllInTimeRange}
