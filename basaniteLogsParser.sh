...
--	This limitation can become a significant issue if a user leaves the company and the administrator needs to access the user’s private filters and dashboards.
...
SELECT count(*) cnt, sr.authorname, cu.display_name
	...
	JOIN cwd_user cu on sr.authorname = CONCAT('JIRAUSER', cu.id) group by sr.authorname, cu.display_name order by cnt desc;
--select * from searchrequest sr; --sr.authorname: JIRAUSER10100
--select * from cwd_user cu;


SELECT * FROM sharepermissions; --288,880 
SELECT distinct (entitytype) FROM sharepermissions; --[PortalPage, SearchRequest]
SELECT distinct (sharetype) FROM sharepermissions; --[loggedin, user, project, group, global]
SELECT * FROM sharepermissions where sharetype='global' Order by RIGHTS; --8,597 --Rights: [1]
SELECT * FROM sharepermissions where sharetype='loggedin' Order by RIGHTS; --68,407 --Rights: [1]
SELECT * FROM sharepermissions where sharetype='project' Order by RIGHTS; --53,854 --Rights:.[1, 3]
SELECT * FROM sharepermissions where sharetype='group' Order by RIGHTS; --84,114 --Rights: [1, 3]
SELECT * FROM sharepermissions where sharetype='user' Order by RIGHTS; --73,903--Rights: [3]

SELECT PARAM1, count (PARAM1) cnt FROM sharepermissions where sharetype='group' group by PARAM1 order by cnt desc;

--Extra:
SELECT sr.id AS filter_id, sr.filtername, u.lower_user_name AS owner, sr.reqcontent AS jql_query
	FROM searchrequest sr left
	JOIN app_user u ON sr.authorname = u.user_key order by owner;
