...
SELECT d.directory_name AS "Directory", u.user_name AS "Username", u.active AS "Active", to_timestamp(CAST(attribute_value AS BIGINT)/1000) AS "Last Login"
	...
	JOIN cwd_directory d ON u.directory_id = d.id ORDER BY "Active", "Last Login" DESC;
--MSSQL:
SELECT d.directory_name AS "Directory", u.user_name AS "Username", u.active AS "Active", DATEADD(second, cast(attribute_value as bigint)/1000,{d '1970-01-01'}) AS "Last Login"
	FROM dbo.cwd_user u
	...
	JOIN dbo.cwd_directory d ON u.directory_id = d.ID ORDER BY "Active", "Last Login" DESC;
